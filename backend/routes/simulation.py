...
def run_custom_simulation():
    ...
    try:
        ...
        if season == 'kharif':
            ...
            climate_risk_msg = f"Monsoon season in {district}: Rainfall ~{rain_val} mm. Good water availability but risk of pests and waterlogging."
        elif season == 'rabi':
            seasonal_yield_mult = 1.05
            seasonal_water_mult = 1.00
            climate_risk_msg = f"Winter season in {district}: Rainfall ~{rain_val} mm. Ideal temperature for high yield and stable growth."
        else:  # summer
            seasonal_yield_mult = 0.85
            seasonal_water_mult = 1.40
            climate_risk_msg = f"Summer season in {district}: Only ~{rain_val} mm rainfall. High temperature stress and extreme water requirement."
        ...
        crop_pest_data  = CROP_PEST_CONTROL.get(crop_type, [{'pest': 'General insects', 'chemical': 'Neem oil 5 ml/L spray', 'organic': 'Pheromone traps + Bt spray'}])
        ...
        return jsonify({
            ...
            'after': {
                ...
                'climate': {
                    ...
                    'insight': "Optimized practices significantly reduce climate-related risks and resource wastage.",
                    'avg_temp': f"{temp_range[0]}°C - {temp_range[1]}°C",
                    'rainfall': f"{rain_val} mm"
                },
